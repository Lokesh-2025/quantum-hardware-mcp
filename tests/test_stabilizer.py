"""
Tests for verify_stabilizer_circuit / verify_stabilizer_hardware_result --
the stabilizer/Clifford checkable-structure verifier ported from
quantum-verifier's core/stabilizer.py.
...
def _dumps(qc: QuantumCircuit) -> str:
...
def test_ghz_matches_known_exact_distribution():
    ...
    result = json.loads(server.verify_stabilizer_circuit(_dumps(qc)))
...
def test_hardware_result_verification_computes_real_fidelity_lower_bound():
    ...
    result = json.loads(server.verify_stabilizer_hardware_result(_dumps(qc), noisy_counts))
    assert result["applicable"]
...
def test_hardware_result_verification_handles_empty_counts():
    ...
    qc.h(0); qc.cx(0, 1)
    qc.measure(range(2), range(2))
    result = json.loads(server.verify_stabilizer_hardware_result(_dumps(qc), {}))
    assert result["applicable"] is False
