"""
qforge.integrals — pure-Python electron integrals and Hartree-Fock.
===================================================================
A self-contained chemistry engine in numpy + scipy alone, so ``qforge`` has no
dependency on PySCF or any other quantum-chemistry package. Handles s and p
orbitals via the McMurchie-Davidson recursion.

Everything is computed FROM GEOMETRY. The only tabulated inputs are STO-3G
basis-set parameters -- the orbital definitions every chemistry code starts
from -- never energies or answers.

Verified against PySCF to ~10 decimal places on H2, LiH, H2O, NH3, CH4, N2,
CO, HCN and formaldehyde.

Two entry points matter to callers:

``integrals(geometry) -> (S, T, V, eri, e_nuc)``
    Overlap, kinetic, nuclear-attraction and two-electron integrals in the
    atomic-orbital basis, plus the classical nuclear repulsion energy.

``rhf(S, T, V, eri, e_nuc, nelec) -> (energy, coefficients, h_core)``
    Restricted Hartree-Fock. This is a MEAN-FIELD reference and a basis
    generator; it does not capture electron correlation, which is precisely
    the part a quantum computer is meant to supply. It also assumes a closed
    shell, so an odd electron count silently uses ``nelec // 2`` doubly
    occupied orbitals -- harmless here, because exact diagonalisation with a
    particle-number penalty fixes the electron count independently of the
    reference (see :mod:`qforge.chemistry`).

Swappable: :func:`qforge.chemistry.build_molecule` accepts any backend
exposing these two functions, so this module can be replaced with PySCF for
larger basis sets without touching the rest of the library.

Basis-set coverage is currently H, He, Li, C, N, O -- enough for the
hydrogen-chain and small-organic benchmarks the library was built against.
Adding an element means adding its STO-3G shell parameters to ``STO3G`` and
its nuclear charge to ``ZTAB``.
"""
import numpy as np
...
STO3G = {
 ...
 'He':[(0,[(6.36242139,0.15432897),(1.158923,0.53532814),(0.31364979,0.44463454)])],
...
ZTAB = {'H':1,'He':2,'Li':3,'C':6,'N':7,'O':8}
