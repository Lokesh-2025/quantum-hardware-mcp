...
- [x] **Shared, live Turso database, so drift detection stops depending on any one machine's laptop being on (2026-08-30).** Both `get_alerts`'s T1/T2 check and `_recent_drift_alert` (the real gate `submit_job`/`ionq_submit_job` check before spending real money — the exact "ibm_boston wasn't recalibrated and nobody knew for 5 hours" scenario this project exists to catch) now read live from a shared Turso database via `turso_db.py`, falling back to the local db automatically when Turso isn't configured. Both the 6-hour local LaunchAgent and the 2-hour GitHub Actions collector now write into it. Same database `quantum-verifier` uses. Scoped to `provider in ('ibm', 'ionq')` — `braket/*` rows (a different access path to overlapping hardware, only ~2 months deep) deliberately excluded from the shared table. `device_alerts` (stored cx/readout-spike/went-offline alerts) stays local-only for now, not yet mirrored to Turso — flagged, not silently done. Found and fixed a real bug along the way: the `libsql_client` package's sync wrapper leaves a background thread that doesn't reliably terminate on process exit (confirmed directly — hangs 90s+ even with `.close()` registered via `atexit`); replaced with a plain `requests`-based HTTP client (`turso_db.py`) instead.
- [x] **Widened the shared Turso schema to the full field set and migrated the real deep history (2026-08-31).** The first pass above only carried the 10-field subset `quantum-verifier` originally used — missing native gate set, CLOPS, quantum volume, calibration timestamps, and 16 other fields this repo's local schema always had. Widened via `ALTER TABLE`, then rebuilt `device_snapshots` fresh from this repo's own local archive (the richer of the two repos' local schemas) and migrated the real deep per-qubit/per-pair history — not the smaller subset from the first pass. **Final counts: device_snapshots 6,081 rows (2022-01-07 → now), qubit_snapshots 692,955 rows, pair_snapshots 746,456 rows.** `device_history`, `device_profile`, and `check_chip_identity`'s `_qubit_fingerprint_vector` now all read live from Turso too, closing the gaps flagged in the entry above — safe now that the full field set is actually there to read. Found and fixed a real performance bug along the way: `execute_batch()` sent each row as its own pipeline step, measuring ~9.7s per 500 rows (~50/sec) — confirmed directly, would have made the full ~1.45M-row migration take ~10 hours. Combining same-SQL rows into one multi-row `INSERT` instead measured ~0.35s for the same 500 rows (~1,400/sec, a ~28x difference), verified correct via direct round-trip checks. The migration itself was interrupted once by a session restart partway through (not a bug) — resumed from near where it stopped rather than re-sending ~1.1M already-inserted rows.
